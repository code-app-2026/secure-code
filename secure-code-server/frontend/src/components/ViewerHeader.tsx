...
export default function ViewerHeader() {
    ...
    useEffect(() => {
        try {
            const sessionToken = sessionStorage.getItem('viewer_accessToken');
    ...
    const handleLogoutConfirm = async () => {
        ...
        sessionStorage.removeItem('viewer_userRole');
        sessionStorage.removeItem('viewer_accessToken');
    ...
    return (
        <>
            <header className="sticky top-0 z-50 w-full h-[60px] bg-[#0a0f1c]/95 backdrop-blur-xl border-b border-slate-800/60 flex items-center justify-between px-6 lg:px-8">
                ...
                {/* Left: Logo and Name */}
                <Link href="/viewer/dashboard" className="flex items-center space-x-2.5 group cursor-pointer">
                ...
                <nav className="hidden md:flex items-center space-x-8 h-full">
                    <Link href="/viewer/dashboard" className={`font-medium text-[13px] h-full flex items-center relative transition-colors ${pathname === '/viewer/dashboard' ? 'text-blue-500' : 'text-slate-400 hover:text-slate-200'}`}>
                        Projects
                        {pathname === '/viewer/dashboard' && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-blue-500 rounded-t-full shadow-[0_-2px_10px_rgba(59,130,246,0.5)]" />}
                    </Link>
                    <Link href="/viewer/password" className={`font-medium text-[13px] h-full flex items-center relative transition-colors ${pathname === '/viewer/password' ? 'text-blue-500' : 'text-slate-400 hover:text-slate-200'}`}>
                        Change Password
                        {pathname === '/viewer/password' && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-blue-500 rounded-t-full shadow-[0_-2px_10px_rgba(59,130,246,0.5)]" />}
                    </Link>
                </nav>
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden fixed top-[60px] left-0 w-full bg-[#0a0f1c]/95 backdrop-blur-xl border-b border-slate-800/60 z-40 shadow-2xl animate-in slide-in-from-top-2 duration-200">
                    <nav className="flex flex-col p-4 space-y-2">
                        <Link onClick={() => setIsMobileMenuOpen(false)} href="/viewer/dashboard" className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${pathname === '/viewer/dashboard' ? 'bg-blue-500/10 text-blue-400' : 'text-slate-300 hover:bg-slate-800/50'}`}>
                            Projects
                        </Link>
                        <Link onClick={() => setIsMobileMenuOpen(false)} href="/viewer/password" className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${pathname === '/viewer/password' ? 'bg-blue-500/10 text-blue-400' : 'text-slate-300 hover:bg-slate-800/50'}`}>
                            Change Password
                        </Link>
                    </nav>
                </div>
