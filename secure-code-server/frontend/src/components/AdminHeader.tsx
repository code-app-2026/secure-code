...
import { Shield, User, LogOut, ChevronDown, Code2, Menu, X, Key, CheckCircle2 } from 'lucide-react';
        ...
        try {
            ...
            if (token) {
                ...
                if (payload.role) {
                    ...
                    setRole(roleStr.toLowerCase() === 'admin' ? 'Admin (SuperUser)' : roleStr);
                }
            }
        } catch (e) {
    ...
    return (
        <>
            <header className="sticky top-0 z-50 w-full h-[60px] bg-[#0a0f1c]/95 backdrop-blur-xl border-b border-slate-800/60 flex items-center justify-between px-6 lg:px-8">
                ...
                {/* Left: Logo and Name */}
                <Link href="/admin/dashboard" className="flex items-center space-x-2.5 group cursor-pointer">
                ...
                <nav className="hidden md:flex items-center space-x-8 h-full">
                    <Link href="/admin/dashboard" className={`font-medium text-[13px] h-full flex items-center relative transition-colors ${pathname === '/admin/dashboard' ? 'text-blue-500' : 'text-slate-400 hover:text-slate-200'}`}>
                        Dashboard
                        {pathname === '/admin/dashboard' && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-blue-500 rounded-t-full shadow-[0_-2px_10px_rgba(59,130,246,0.5)]" />}
                    </Link>
                    <Link href="/admin/users" className={`font-medium text-[13px] h-full flex items-center relative transition-colors ${pathname === '/admin/users' ? 'text-blue-500' : 'text-slate-400 hover:text-slate-200'}`}>
                        Users
                        {pathname === '/admin/users' && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-blue-500 rounded-t-full shadow-[0_-2px_10px_rgba(59,130,246,0.5)]" />}
                    </Link>
                    <Link href="/admin/projects" className={`font-medium text-[13px] h-full flex items-center relative transition-colors ${pathname === '/admin/projects' ? 'text-blue-500' : 'text-slate-400 hover:text-slate-200'}`}>
                        Project
                        {pathname === '/admin/projects' && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-blue-500 rounded-t-full shadow-[0_-2px_10px_rgba(59,130,246,0.5)]" />}
                    </Link>
                    <Link href="/admin/sessions" className={`font-medium text-[13px] h-full flex items-center relative transition-colors ${pathname === '/admin/sessions' ? 'text-blue-500' : 'text-slate-400 hover:text-slate-200'}`}>
                        Sessions
                        {pathname === '/admin/sessions' && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-blue-500 rounded-t-full shadow-[0_-2px_10px_rgba(59,130,246,0.5)]" />}
                    </Link>
                    ...
                    <Link href="/admin/settings" className={`font-medium text-[13px] h-full flex items-center relative transition-colors ${pathname === '/admin/settings' ? 'text-blue-500' : 'text-slate-400 hover:text-slate-200'}`}>
                        Account Settings
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden fixed top-[60px] left-0 w-full bg-[#0a0f1c]/95 backdrop-blur-xl border-b border-slate-800/60 z-40 shadow-2xl animate-in slide-in-from-top-2 duration-200">
                    <nav className="flex flex-col p-4 space-y-2">
                        <Link onClick={() => setIsMobileMenuOpen(false)} href="/admin/dashboard" className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${pathname === '/admin/dashboard' ? 'bg-blue-500/10 text-blue-400' : 'text-slate-300 hover:bg-slate-800/50'}`}>
                            Dashboard
                        </Link>
                        <Link onClick={() => setIsMobileMenuOpen(false)} href="/admin/users" className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${pathname === '/admin/users' ? 'bg-blue-500/10 text-blue-400' : 'text-slate-300 hover:bg-slate-800/50'}`}>
                            Users
                        </Link>
                        <Link onClick={() => setIsMobileMenuOpen(false)} href="/admin/projects" className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${pathname === '/admin/projects' ? 'bg-blue-500/10 text-blue-400' : 'text-slate-300 hover:bg-slate-800/50'}`}>
                            Project
                        </Link>
                        <Link onClick={() => setIsMobileMenuOpen(false)} href="/admin/sessions" className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${pathname === '/admin/sessions' ? 'bg-blue-500/10 text-blue-400' : 'text-slate-300 hover:bg-slate-800/50'}`}>
                            Sessions
                        </Link>
